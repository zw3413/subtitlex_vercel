...
export default function SubtitleRating({ rating }) {
  useEffect(() => {
    console.log("DOMContentLoaded");
    console.log(rating);
    for (var i = 1; i <= 5; i++) {
      document.querySelector("#hs-ratings-readonly-" + i).checked = false;
    }
    let no = Math.floor((10 - rating) / 2) + 1;
    if (no > 0 && no <= 5) {
      document.querySelector("#rating").classList.remove("hidden");
      document.querySelector("#hs-ratings-readonly-" + no).checked = true;
    }
  }, []);

  const calcRating = (starNo) => {
    return 10 - (starNo - 1) * 2;
  };
